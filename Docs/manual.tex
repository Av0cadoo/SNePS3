...
\title{\bf SNePS~3 USER'S MANUAL\thanks{The development of SNePS was supported
...
&& $\mid$ `(' , `thresh' , `(' , $i$ , `)' wft$^+$ , `)'\\
&& $\mid$ \textcolor{red}{`(', `close' , (atomicname $\mid$ (`(',
  atomicname$^+$ , `)')) ,}\\
&& ~~~~~\textcolor{red}{wft , `)'}\\
&& $\mid$ `(', `every' , atomicname , wft$^\ast$ , `)'\\
&& $\mid$ `(', `some' , atomicname , `(' , wft$^\ast$ , `)' ,
 wft$^\ast$ , `)'\\ 
...
\textit{Lisp number} ;\\
[2ex]
atomicname &=& \textit{Lisp symbol other than} \texttt{wft}$i$ ;\\
[2ex]
...
&& $\mid$ `(' , `restrict' , path , (atomicwft $\mid$ `wft'$\not b$  $i$) , `)'
\end{tabular}}
...
\noindent\ct{.+.}, \ct{.-.}, \ct{.*.}, \ct{./.}
\indexct{.+.} \indexct{.-.} \indexct{.*.} \indexct{./.}
 \hfill [\textit{Function}]\newline
Each function takes an arbitrary number of arguments, each of which can be a
number or a term that looks like a number.  The function
\texttt{unbox}es\indexct{unbox} each 
of its arguments, applies the corresponding Lisp arithmetic function, and
returns the \texttt{box}ed\indexct{box} version of the result.

\vspace{3ex}\noindent\ct{.<.}, \ct{.<=.}, \ct{.>.}, \ct{.>=.}, \ct{.=.}, \ct{./=.}
\indexct{.<.} \indexct{.<=.} \indexct{.>.} \indexct{.>=.} \indexct{.=.} \indexct{./=.}
 \hfill [\textit{Function}]\newline
 Each function takes an arbitrary number of arguments, each of which can be a
 number or a term that looks like a number.  The function
 \texttt{unbox}es\indexct{unbox} each of its arguments, applies the
 corresponding Lisp relational function, and returns \texttt{t} or \texttt{nil},
 appropriately.

\docfun{allTerms}{\&key (test \#'(lambda (x) t))}
Returns a set of all the terms in the knowledge base that satisfy the test,
which defaults to the always True function.

\docfun{ask}{\textit{exprpat}}
Returns a set of instances of the term pattern \cv{exprpat} or its negation
that are derivable in the current context; or the empty set if there are none.

\docfun{askif}{\textit{exprpat}}
Returns a set of instances of the term pattern \cv{exprpat} that are
derivable in the current context; or the empty set if there are none.

\docfun{askifnot}{\textit{exprpat}}
"Returns a set of instances of the negation of the term pattern
\cv{exprpat} that are derivable in the current context; or
...
\docfun{box}{\textit{n}}
Returns a term whose name looks like \cv{n}, \texttt{build}ing it if necessary.

\docfun{clearkb}{\&optional (\textit{clearall} nil)}
Reinitializes the SNePS knowledge base.  If \cv{clearall} is
non-\texttt{nil} also reinitializes all slots, and
caseframes, but not the semantic types.
...
types listed in the list \cv{supers}.  If \cv{docstring} is given, it
is set as the documentation string of the new type.  Returns a
string-message, either of success or what the problem was.

\docfun{demo}{\&key file pause}
...
omitted, a menu will be presented of available demos.
...
Returns two values: a set of instances of \textit{exprpat} that are in the
knowledge base; and a set of substitutions, which when applied to
\textit{exprpat} would give those instances.  \textit{exprpat} may be any wft
with variables, symbols starting with a ``?'', in the place of any subterms.

\docfun{find-term}{\textit{name}}
...
\docvar{KRNovice}
If set to a non-\texttt{null} value (the default value is \texttt{nil}), slots
...
\docfun{list-contexts}{}
Prints a list of all the contexts.

\docfun{list-slots}{}
Prints a list of all the SNePS slots.

\docfun{list-terms}{\&key (asserted nil) (types nil)}
Prints a list of all the terms in the KB.  If \texttt{asserted} is non-null,
only asserted propositions will be printed; otherwise, all terms will
be printed.  If \texttt{:types} is non-null, the type of
each term will also be printed.
...
\docmacro{startGUI}{\&rest \textit{terms}} 
  Starts the SNePS 3 GUI. Takes a variable number of \texttt{\textit{terms}} to
...
\docfun{unbox}{term}
If \cv{term} is a number, return it; if \cv{term}'s name looks like a number,
return the number; else throw an error.
...
\bibliography{/u0/csefaculty/shapiro/Util/Latex/bibliography}
